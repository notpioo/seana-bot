...
async function plantHandler(sock, msg) {
    try {
        ...
        if (!plantName || isNaN(amount) || amount <= 0) {
            ...
            const plantStatus = farm.plants
                .map((plant) => {
                    ...
                    // Apply pet growth speed bonus first
                    const speedBonus = plant.speedBonus || 0;
                    if (speedBonus > 0) {
                        growthTime = Math.floor(growthTime * (1 - (speedBonus / 100)));
                    }

                    // Then apply weather effects
                    if (currentWeather.type === "rain") {
                        growthTime = Math.floor(growthTime * 0.8); // 20% faster
                    } else if (currentWeather.type === "snow") {
                        growthTime = Math.floor(growthTime * 1.5); // 50% slower
                    }

                    // Finally apply greenhouse bonus if exists
                    const greenhouse = farm.greenhouse.find(
                        (gh) => gh.slot === plant.slot,
                    );
                    if (greenhouse) {
                        growthTime = Math.floor(growthTime * (1 - greenhouse.growthBonus / 100));
                    }
                    ...
                    const bonusText = [];
                    if (speedBonus > 0) bonusText.push(`+${speedBonus}% 🐾`);
                    if (greenhouse) bonusText.push(`+${greenhouse.growthBonus}% 🏘️`);

                    return `${icon} ${plant.name} x${plant.quantity} (⏳ ${timeLeft}s dari ${growthTime}s)${bonusText.length > 0 ? ` [${bonusText.join(', ')}]` : ''}`;
                })
                .join("\n");
        ...
        let updatedFarm = await initFarm(senderJid);
        if (!updatedFarm) {
            await sock.sendMessage(msg.key.remoteJid, {
        ...
        // Get active pet info for growth speed bonus
        const activePet = farm.activePet;
        const petInfo = activePet ? PETS[activePet.name.toLowerCase()] : null;
        let growthSpeed = 0;


        // Calculate adjusted growth time with pet bonus
        const baseGrowthTime = PLANTS[plantName].growthTime;
        const growthSpeedBonus = petInfo ? petInfo.effects.growthSpeed : 0;
        const adjustedGrowthTime = Math.floor(baseGrowthTime * (1 - (growthSpeedBonus / 100)));

        // Add plant with adjusted growth time
        farm.plants.push({
            name: plantName,
            plantedAt: new Date(),
            quantity: amount,
            slot: usedSlots + 1,
            adjustedGrowthTime: adjustedGrowthTime,
            speedBonus: growthSpeedBonus
        });
        ...
        const plantStatus = farm.plants
            .map((plant) => {
                // Calculate remaining time based on adjusted growth time
                const baseTime = PLANTS[plant.name].growthTime;
                const speedBonus = plant.speedBonus || 0;
                const adjustedTime = Math.floor(baseTime * (1 - (speedBonus / 100)));
                ...
                const speedBonusText = speedBonus > 0 ? ` [+${speedBonus}% speed]` : '';
                const icon = "🌱";
                return `${icon} ${plant.name} x${plant.quantity} (⏳ ${timeLeft}s dari ${adjustedTime}s)${speedBonusText}`;
            })
            .join("\n");
...
async function harvestHandler(sock, msg) {
    try {
        ...
        const now = new Date();
        // Get active pet info once
        const activePet = farm.activePet;
        const petInfo = activePet ? PETS[activePet.name.toLowerCase()] : null;
        let expBonus = 0;
        let sellBonus = 0;

        // Calculate pet bonuses if pet has energy
        if (petInfo ) {
            expBonus = petInfo.effects.expGain;
            sellBonus = petInfo.effects.sellBonus;
        }

        const readyToHarvest = farm.plants.filter((plant) => {
            let growthTime = (plant.adjustedGrowthTime || PLANTS[plant.name].growthTime) * 1000; // convert to ms
        ...
        readyToHarvest.forEach((plant) => {
            ...
            // Consume pet energy once per harvest action
            if (activePet ) {
            ...
            for (let i = 0; i < plant.quantity; i++) {
                ...
                for (let j = 0; j < harvestCount; j++) {
                    ...
                    if (Math.random() <= diamondChances[plantInfo.tier]) {
                        ...
                        harvests.push(
                            `${weight}kg (${plantInfo.tier})${sellBonus ? ` [+${sellBonus}% 💰]` : ''} + 💎${diamondAmount}`,
                        );
                    } else {
                        harvests.push(`${weight}kg (${plantInfo.tier})`);
                    }
                    const baseXP = plantInfo.xpGain;
                    const bonusXP = Math.floor(baseXP * (expBonus / 100));
                    totalXP += baseXP;
                    if (bonusXP > 0) {
                        totalXP += bonusXP;
                    }
        ...
        const baseXPDisplay = totalXP - Math.floor(totalXP * (expBonus / 100));
        const bonusXPDisplay = Math.floor(totalXP * (expBonus / 100));
        harvestReport += `✨ XP didapat: ${baseXPDisplay}${bonusXPDisplay > 0 ? ` (+${bonusXPDisplay} 🐾)` : ''}\n`;
...
async function farmBagHandler(sock, msg) {
    try {
        ...
        // Display pets
        const PET_NAMES = ["fluff", "shadow", "celestia", "celestial"];
        const petSeeds = farm.seeds.filter(seed => PET_NAMES.includes(seed.name.toLowerCase()));
        if (petSeeds.length > 0) {
            inventoryText += "\n🐾 *PETS*\n";
            petSeeds.forEach((pet) => {
                const petInfo = PETS[pet.name.toLowerCase()];
                inventoryText += `├ ${pet.name} (${petInfo.tier})\n`;
                inventoryText += `│ └ 📦 Jumlah: ${pet.quantity}\n`;
            });
        }

        // Display seeds (excluding pets)
        const actualSeeds = farm.seeds.filter(seed => !PET_NAMES.includes(seed.name.toLowerCase()));
        if (actualSeeds.length > 0) {
            inventoryText += "\n🌱 *SEEDS*\n";
...
async function sellAllHandler(sock, msg) {
    try {
        ...
        farm.inventory.crops.forEach((crop) => {
            ...
            // Apply pet effects if active and has energy
            const activePet = farm.activePet;
            const petInfo = activePet ? PETS[activePet.name.toLowerCase()] : null;
            let petBonus = 0;

            if (petInfo && petInfo.effects.sellBonus > 0) {
                petBonus = Math.floor(profit * (petInfo.effects.sellBonus / 100));
                profit += petBonus;

            }

            soldItems.push(
                `${crop.name} (${crop.weight.toFixed(2)}kg) - 💰${profit - petBonus} (+${petBonus} 🐾)`
            );
...
async function fpetHandler(sock, msg) {
    try {
        ...
        if (petName) {
            const pet = farm.pets.find(p => p.name.toLowerCase() === petName.toLowerCase());
            if (!pet) {
                await sock.sendMessage(msg.key.remoteJid, {
            ...
            await sock.sendMessage(msg.key.remoteJid, {
                text: `✅ Pet ${pet.name} berhasil diaktifkan!`,
                quoted: msg
            });
            return;
        }
        ...
        const petDisplay = farm.pets.map(pet => {
            ...
            return `🐾 ${pet.name} (${petInfo.tier}) ${isActive}\n` +
...
async function fgiveHandler(sock, msg) {
    try {
        ...
        // Handle pet giving
        if (["fluff", "shadow", "celestial"].includes(item.toLowerCase())) {
            const pet = {
