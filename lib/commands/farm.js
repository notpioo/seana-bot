    ...
    try {
        ...
        const plantStatus = farm.plants
            .map((plant) => {
                const timeLeft = Math.max(
                    0,
                    (plant.adjustedGrowthTime || PLANTS[plant.name].growthTime) -
                        Math.floor(
                ...
                const speedBonusText = plant.speedBonus ? ` [+${plant.speedBonus}% speed]` : '';
                const icon = "🌱";
                return `${icon} ${plant.name} x${plant.quantity} (⏳ ${timeLeft}s)${speedBonusText}`;
            })
            .join("\n");
