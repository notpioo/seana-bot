...
async function plantHandler(sock, msg) {
    try {
        ...
        // Add plants with growth speed bonus if applicable
        farm.plants.push({
            name: plantName,
            plantedAt: new Date(),
            quantity: amount,
...
async function harvestHandler(sock, msg) {
    try {
        ...
        const readyToHarvest = farm.plants.filter((plant) => {
            ...
            // Apply pet growth speed bonus if pet is active and has energy
            if (petInfo && activePet.energy > 0 && petInfo.effects.growthSpeed > 0) {
                growthTime *= (1 - petInfo.effects.growthSpeed / 100);
            }
        ...
        // Remove harvested plants and add XPfarm.plants = farm.plants.filter(
