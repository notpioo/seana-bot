    ...
    try {
        ...
        const readyToHarvest = farm.plants.filter((plant) => {
            ...
            if (activePet && activePet.energy > 0) {
                const petInfo = PETS[activePet.name.toLowerCase()];
                if (petInfo && petInfo.effects.growthSpeed > 0) {
        ...
        readyToHarvest.forEach((plant) => {
            ...
            const sellBonus = (petInfo && activePet.energy > 0) ? petInfo.effects.sellBonus : 0;
            const expBonus = (petInfo && activePet.energy > 0) ? petInfo.effects.expGain : 0;
        ...
        // Consume pet energy if active
        if (farm.activePet && farm.activePet.energy > 0) {
            farm.activePet.energy = Math.max(0, farm.activePet.energy - readyToHarvest.length);
            harvestReport += `\n🐾 Pet energy: ${farm.activePet.energy}/100\n`;
        }
