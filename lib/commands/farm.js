...
function refreshStock(force = false) {
    ...
    if (force || now - lastRefresh >= REFRESH_INTERVAL) {
        ...
        Object.entries(tierChances).forEach(([tier, chance]) => {
            if (Math.random() <= chance && plantsByTier[tier]) {
                const tierPlants = plantsByTier[tier];
                const plantsToAdd = Math.floor(Math.random() * 3) + 1; // 1-3 plants per tier

                for (let i = 0; i < plantsToAdd && tierPlants.length > 0; i++) {
                    const randomIndex = Math.floor(Math.random() * tierPlants.length);
                    const plantName = tierPlants[randomIndex];
                    tierPlants.splice(randomIndex, 1);
        ...
        // Get remaining plant names that weren't distributed in tier-based distribution
        const plantNames = Object.keys(PLANTS).filter(name => !shopStock[name]);

        // Randomly distribute remaining stock
...
async function harvestHandler(sock, msg) {
    try {
        ...
        if (currentWeather.type === 'storm') {
            farm.plants = farm.plants.filter(plant => Math.random() > 0.2); // 20% chance to destroy plants
        }

        // Diamond chance based on tier
        const diamondChances = {
            'common': 0.01,    // 1% chance
            'rare': 0.02,      // 2% chance
            'epic': 0.03,      // 3% chance
            'legendary': 0.05, // 5% chance
            'mythical': 0.07,  // 7% chance
            'divine': 0.10     // 10% chance
        };
...
async function fupgradeHandler(sock, msg){
    try{
        ...
        const upgradeOptions = [`Tanah (+1 slot, 30000 coins)`];
        const upgradeMessage = `Upgrade Tersedia:\n\n${upgradeOptions.map((option, index) => `${index + 1}. ${option}`).join('\n')}\n\nKetik .fupland untuk mengupgrade tanah.`;
...
async function fuplandHandler(sock, msg){
    try{
        ...
        await sock.sendMessage(msg.key.remoteJid, { text: `✅ Upgrade tanah berhasil! Slot tanah sekarang ${farm.slots}`, quoted: msg });
    } catch(error){
