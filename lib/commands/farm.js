...
async function plantHandler(sock, msg) {
    try {
        ...
        // Apply pet growth speed if pet has energy
        if (petInfo && activePet.energy > 0 && petInfo.effects.growthSpeed > 0) {
            growthSpeed = petInfo.effects.growthSpeed;
            // Consume pet energy
            activePet.energy = Math.max(0, activePet.energy - 1);
        }
        ...
        const adjustedGrowthTime = baseGrowthTime * (1 - (growthSpeedBonus / 100));
...
async function sellAllHandler(sock, msg) {
    try {
        ...
        farm.inventory.crops.forEach((crop) => {
            ...
            if (petInfo && activePet.energy > 0 && petInfo.effects.sellBonus > 0) {
                petBonus = Math.floor(profit * (petInfo.effects.sellBonus / 100));
                profit += petBonus;
                // Consume pet energy for using sell bonus ability
                activePet.energy = Math.max(0, activePet.energy - 1);
            }
