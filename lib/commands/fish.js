...
function generateCatches(maxCatch, availableFish) {
    const catches = [];
    ...
    for(let i = 0; i < numCatches; i++) {
        ...
        // First determine rarity based on chances
        for(const [rarity, chance] of Object.entries({
            ...
            if(rand <= cumulativeChance && fishByRarity[rarity].length > 0) {
                // Randomly select a fish from the chosen rarity group
        ...
        // Fallback to common fish if no fish was selected
        if (!selectedFish && fishByRarity.Common.length > 0) {
            selectedFish = fishByRarity.Common[Math.floor(Math.random() * fishByRarity.Common.length)];
        }
...
async function fishingHandler(sock, msg) {
    try {
        ...
        // Apply bait effects to catch chances
        const baitModifiedChances = {
            Common: getRarityChance('Common'),
            Uncommon: getRarityChance('Uncommon') + (baitData.effects.uncommon || 0),
            Rare: getRarityChance('Rare') + (baitData.effects.rare || 0),
            Epic: getRarityChance('Epic') + (baitData.effects.epic || 0),
            Legendary: getRarityChance('Legendary') + (baitData.effects.legendary || 0)
        };

        // Generate catches with modified chances
        const catches = generateCatches(rodData.maxCatch, areaData.fish);
...
async function dashboardHandler(sock, msg) {
    try {
        ...
        `${BAITS[fishData.currentBait].name} (${fishData.inventory.baits.find(b => b.name === fishData.currentBait)?.quantity || 0} left)` : 
        'Tidak ada';
    
    const dashboard = `🎣 *FISHING DASHBOARD*\n` +
        `👤 Player: @${senderJid.split('@')[0]}\n\n` +
        `🗺️ Area: ${areaData.name}\n` +
        `🎣 Rod: ${currentRod?.name || 'Tidak ada'}\n` +
        `🪱 Bait: ${currentBait}\n` +  // Added this line
        `💪 Durability: ${currentRod?.durability || 0}\n` +
            `🏆 Total Catch: ${fishData.stats.totalCatch}\n` +
