...
const AREA_REQUIREMENTS = {
    ...
    singapore: {
        price: 40000,
        fishingCount: 600,
        requiredFish: 'toman' // ikan legendary dari malaysia
    },
    goa_berair: {
        price: 55000,
        fishingCount: 950,
...
const BAITS = {
    normal: {
        ...
        price: 5000,
        quantity: 50,
        areas: ['indonesia', 'malaysia', 'singapore', 'goa_berair', 'pantai', 'laut'], // Bisa dipakai di semua area
        effects: {
            rare: 0,
            epic: 0,
            legendary: 0,
    ...
    daging: {
        ...
        price: 30000,
        quantity: 30,
        areas: ['indonesia'], // Cocok untuk area menengah
        effects: {
            rare: 0,
            epic: 0.15,
            legendary: 0.05,
        ...
        description: 'Umpan daging untuk area indonesia'
    },
    emas: {
        name: 'Umpan Emas',
        price: 100000,
        quantity: 5,
        areas: ['laut','indonesia'], // Khusus untuk area laut
        effects: {
            rare: 0,
            epic: 0,
            legendary: 0.1,
        ...
        description: 'Umpan spesial khusus area laut, meningkatkan kemungkinan mendapat ikan legendary, mythic, dan secret'
    }
};
...
const AREAS = {
    indonesia: {
        name: 'Indonesia',
        fish: [
            { name: 'Lele', rarity: 'Common', price: 20 },
            ...
            { name: 'Belida', rarity: 'Mythic', price: 720 },
            { name: 'Batak', rarity: 'Mythic', price: 740 },
            { name: 'Semah', rarity: 'Mythic', price: 760 },
            { name: 'Hiu Gergaji', rarity: 'Mythic', price: 780 },
            { name: 'Kancra', rarity: 'Mythic', price: 800 },

            { name: 'Balashark', rarity: 'Legendary', price: 900 },
            { name: 'Megalodon', rarity: 'Legendary', price: 1000 }
        ]
    },
    malaysia: {
        name: 'Malaysia',
        fish: [
            ...
            { name: 'Siakap', rarity: 'Mythic', price: 600 },
            { name: 'Jenahak', rarity: 'Mythic', price: 600 },
            { name: 'Temoleh', rarity: 'Mythic', price: 600 },
            { name: 'Hiu Gergaji', rarity: 'Mythic', price: 600 },
            { name: 'Kancra', rarity: 'Mythic', price: 600 },

            { name: 'Tapah', rarity: 'Legendary', price: 600 },
            { name: 'Kelisa', rarity: 'Legendary', price: 600 }
        ]
    },
    singapore: {
        name: 'Singapore',
...
function getRarityChance(rarity) {
    switch(rarity) {
        ...
        case 'Legendary': return 0.009;   // Diturunkan dari 0.01
        case 'Mythic': return 0.0025;    // Rarity baru (0.25%)
...
function generateCatches(maxCatch, availableFish, baitData) {
    ...
    for(let i = 0; i < numCatches; i++) {
        ...
        const modifiedChances = {
            ...
            Legendary: 0.009 + (baitData.effects.legendary || 0),
            Mythic: 0.0025 + (baitData.effects.mythic || 0),
...
async function fishBagHandler(sock, msg) {
    try {
        ...
        const baitInventory = fishData.inventory.baits
            .map(bait => {
                if (!BAITS[bait.name]) return null; // Skip jika bait tidak valid
                return `🪱 ${BAITS[bait.name].name} (${bait.quantity})`
            })
        ...
        const rodInventory = fishData.inventory.rods
            .map(rod => {
                // Validasi rod dan rodData
                if (!rod || !rod.name || !RODS[rod.name]) return null;
                const rodData = RODS[rod.name];
                
                // Validasi durability
                if (typeof rod.durability !== 'number' || typeof rodData.durability !== 'number') return null;

                // Menghitung persentase durability
                const maxDurability = rodData.durability;
                const currentPercentage = (rod.durability / maxDurability) * 100;
                
                // Memilih emoji berdasarkan persentase durability
                let durabilityEmoji;
...
async function switchBaitHandler(sock, msg) {
    try {
        ...
        // Check if bait can be used in current area
        const baitData = BAITS[baitName];
        if (!baitData.areas.includes(fishData.currentArea)) {
            await sock.sendMessage(msg.key.remoteJid, {
