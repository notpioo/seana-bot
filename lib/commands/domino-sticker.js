...
const findMatchingCardInHand = (playerCards, detectedValues) => {
    ...
    for (let i = 0; i < playerCards.length; i++) {
        ...
        const cardLeft = Number(card.left);
        const cardRight = Number(card.right);
        const detectedLeft = Number(detectedValues.left);
        const detectedRight = Number(detectedValues.right);

        // Check both orientations
        if ((cardLeft === detectedLeft && cardRight === detectedRight) ||
            (cardLeft === detectedRight && cardRight === detectedLeft)) {
            return {
                found: true,
                index: i,
                card: card
