...
async function handleGameTimeout(roomId) {
    ...
    if (winner && timedOutPlayer) {
        const winnerUser = User.getUser(winner);
        const loserUser = User.getUser(timedOutPlayer);
        ...
        if (loserUser) {
            ...
            User.updateUser(timedOutPlayer, { balance: loserUser.balance });
        }
    }
        ...
        if (gameStatus) {
            if (gameStatus === 'draw') {
                ...
                for (const playerJid of [currentGame.players.X, currentGame.players.O]) {
                    const user = User.getUser(playerJid);
                    if (user) {
                        user.balance += 75;
                        const updatedUser = User.updateUser(playerJid, { balance: user.balance });
                        gameEndMessage += `@${playerJid.split('@')[0]} mendapat 75 balance (Total: ${updatedUser.balance})\n`;
                    }
                }
                ...
                const winnerUser = User.getUser(winner);
                const loserUser = User.getUser(loser);

                if (winnerUser) {
                    winnerUser.balance += 400;
                    const updatedWinner = User.updateUser(winner, { balance: winnerUser.balance });
                    gameEndMessage += `🎉 @${winner.split('@')[0]} menang! (+400 balance, Total: ${updatedWinner.balance})\n`;
                }

                if (loserUser) {
                    loserUser.balance += 50;
                    const updatedLoser = User.updateUser(loser, { balance: loserUser.balance });
                    gameEndMessage += `@${loser.split('@')[0]} kalah (+50 balance, Total: ${updatedLoser.balance})`;
                }
            }
