...
const GENERAL_FISH_POOL = Object.entries(FISH_DATA).reduce((pool, [rarity, fishes]) => {
    return [...pool, ...Object.entries(fishes).map(([key, fish]) => ({
        ...fish,
        key,
        rarity
    }))];
...
async function startFishing(sock, msg) {
    ...
    try {
        ...
        if (!fishData.location || !fishData.location.currentLocation) {
            fishData.location = {
                currentLocation: 'danau',
                unlockedLocations: {
                    danau: true,
        ...
        if (!fishData.equipment || !fishData.equipment.currentRod) {
            fishData.equipment = {
                currentRod: 'bambu',
                rods: { bambu: true }
            };
        ...
        // Check cooldown with proper error handling
        const now = Date.now();
        ...
        if (!rod) {
            await sock.sendMessage(msg.key.remoteJid, {
                text: '❌ Error: Pancingan tidak valid. Gunakan .fishstats untuk cek pancingan Anda.',
                quoted: msg
            });
            return;
        }
        ...
        // Get current location data with error handling
        const location = FISHING_LOCATIONS[fishData.location.currentLocation];
        if (!location) {
            await sock.sendMessage(msg.key.remoteJid, {
                text: '❌ Error: Lokasi tidak valid. Gunakan .location untuk mengubah lokasi.',
                quoted: msg
            });
            return;
        }
        ...
        // Update lastFished immediately to prevent spam
        ...
        for (let i = 1; i < FISHING_ANIMATION.length; i++) {
            ...
            }).catch(console.error); // Add error handling for message edits
        }

        // Catch fish with proper pool handling
        ...
        for (let i = 0; i < numCatch; i++) {
            ...
            // Update inventory
            fishData.inventory = fishData.inventory || {};
            fishData.inventory[fish.key] = (fishData.inventory[fish.key] || 0) + 1;
            
            // Update stats
            fishData.stats.totalCatch++;
            if (fish.rarity === 'rare') fishData.stats.rareCatch++;
            if (fish.rarity === 'epic') fishData.stats.epicCatch++;
            if (fish.rarity === 'legendary') fishData.stats.legendaryCatch++;
        }

        await fishData.save();

        // Format catch message
        const catchText = catches.map(fish => `${fish.emoji} ${fish.name} (${formatNumber(fish.price)} balance)`).join('\n');
        await sock.sendMessage(msg.key.remoteJid, {
            text: `🎣 Hasil Memancing:\n\n${catchText}\n\nTotal: ${formatNumber(totalValue)} balance`,
            quoted: msg
        });
