        ...
        if (isVideo) {
            ffmpegCommand
                .addOutputOptions([
                    ...
                    '-loop', '0',         // Loop tanpa batas
                    '-preset', 'default', // Preset untuk kualitas yang baik
                    '-an',               // Hapus audio
                    '-vsync', '0',       // Sinkronisasi video
                    '-t', '10',          // Maksimum 10 detik
                    '-compression_level', '6', // Level kompresi (0-6)
                    '-quality', '50',    // Kualitas WebP (0-100)
                    '-lossless', '0',    // Non-lossless untuk ukuran lebih kecil
                    '-b:v', '256k'       // Bitrate video
                ])
                .size('512x512')
                .aspect('1:1')
                .fps(12)
...
async function stickerHandler(sock, msg) {
    try {
        ...
        try {
            ...
            if (stats.size > 1000000) { // Jika lebih dari 1MB
                throw new Error('Sticker terlalu besar! Coba video yang lebih pendek atau gambar yang lebih kecil.');
            }
