...
async function getOpenAIInstance() {
    const apiKeyDoc = await ApiKey.findOne({ name: 'openai' });
    if (!apiKeyDoc || !apiKeyDoc.key) {
        throw new Error('API key belum diatur. Gunakan .setapikey untuk mengatur API key.');
    }
...
async function seaHandler(sock, msg) {
    try {
        ...
        // Get OpenAI instance with current API key
