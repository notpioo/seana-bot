...
const startDomHandler = async (sock, msg) => {
    try {
        ...
        dominoGames[groupJid].currentPlayerIndex = Math.floor(Math.random() * dominoGames[groupJid].players.length);
...
const processPlayedCard = async (sock, msg) => {
    try {
        ...
        if (dominoGames[groupJid].playedCards.length === 0) {
            ...
            currentPlayer.cards = currentPlayer.cards.filter((_, i) => i !== index);

            // Send updated cards to player in private chat
            const updatedCardsMessage = `🎮 *SISA KARTU KAMU*\n\n${currentPlayer.cards.map((card, index) =>
                `${index + 1}. ${card.left} | ${card.right}`).join('\n')}\n\nGunakan sticker domino saat giliranmu.`;

            await sock.sendMessage(currentPlayer.jid, {
                text: updatedCardsMessage
            });
        } else {
...
// Handle domino help command
const domHelpHandler = async (sock, msg) => {
    try {
        const helpText = `🎮 *TUTORIAL BERMAIN DOMINO*

*1. Membuat Room*
➣ Ketik .domino untuk membuat room baru
➣ Room akan kadaluarsa dalam 5 menit jika tidak dimulai

*2. Bergabung ke Room*
➣ Ketik .joindom untuk bergabung ke room
➣ Minimal 2 pemain, maksimal 4 pemain
➣ Setiap pemain akan mendapat 7 kartu

*3. Memulai Permainan*
➣ Pembuat room ketik .startdom untuk mulai
➣ Kartu akan dikirim ke private chat masing-masing

*4. Cara Bermain*
➣ Gunakan sticker domino untuk mengeluarkan kartu
➣ Kartu harus sesuai dengan angka di ujung papan
➣ Jika tidak punya kartu yang cocok:
   • Ketik .dom untuk ambil kartu baru
   • Ketik .dompass untuk lewati giliran (jika deck habis)

*5. Aturan Penting*
➣ Permainan bergiliran sesuai urutan
➣ Kartu pertama bebas (tidak ada syarat)
➣ Pemain yang habis kartunya duluan menang
➣ Pemenang mendapat 500 balance

*6. Tips*
➣ Perhatikan kartu yang sudah keluar
➣ Manfaatkan .dom untuk mencari kartu yang cocok
➣ Jangan lupa cek sisa kartu lawan
...
*Selamat bermain! 🎲*`;
        ...
        console.error("Error in domHelpHandler:", error);
        await sock.sendMessage(msg.key.remoteJid, {
