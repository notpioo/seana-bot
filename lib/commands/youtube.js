...
async function youtubeHandler(sock, msg) {
    try {
        ...
        if (response.data && response.data.data) {
            ...
            if (videoData.channel?.name) caption += `👤 Author: ${videoData.channel.name}\n`;
            if (videoData.lengthText) caption += `⏱️ Duration: ${videoData.lengthText}\n`;
            
            caption += `\n_Video ini didownload menggunakan SeanaBot_`;

            // Get highest quality thumbnail URL as fallback
            const thumbnailUrl = videoData.thumbnails?.slice(-1)[0]?.url;
