    ...
    try {
        ...
        if (room.setChoice(senderJid, choice)) {
            ...
            if (room.allPlayersChosen()) {
                ...
                // Find the player with the largest bet
                const largestBetPlayer = room.players.reduce((prev, current) =>
                    (prev.bet > current.bet) ? prev : current
                );

                // Determine the forced result based on the largest bet player's choice
                const forcedResult = largestBetPlayer.choice === 'B' ? 'K' : 'B';

                const { total, emojis } = room.diceResult;

                let resultMessage = `🎲 Hasil Dadu:\n${emojis.join(' ')} = ${total}\n` +
                                  `Hasil: ${forcedResult === 'B' ? 'BESAR 📈' : 'KECIL 📉'}\n\n` +
                                  `Detail Permainan:\n`;

                // Process winners and losers based on the forced result
                for (const player of room.players) {
                    const user = await User.getUser(player.jid);
                    const isWinner = player.choice === forcedResult;
                ...
                resultMessage += `Terima kasih telah bermain! 🎮\n` +
                                `Pemain dengan taruhan terbesar (@${largestBetPlayer.jid.split('@')[0]}) dipastikan kalah! 💥`;
