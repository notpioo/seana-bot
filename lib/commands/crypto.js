...
async function buyRamHandler(sock, msg) {
    try {
        ...
        if (ramSize > crypto.ram.size + 1) {
            await sock.sendMessage(msg.key.remoteJid, {
                text: `❌ Kamu harus upgrade RAM secara bertahap!\nRAM saat ini: ${crypto.ram.size}GB\nKamu hanya bisa upgrade ke ${crypto.ram.size + 1}GB`,
                quoted: msg
            });
            return;
        }
        ...
        if (user.balance < ramConfig.price) {
            const formatPrice = ramConfig.price.toLocaleString();
            const formatBalance = user.balance.toLocaleString();
            await sock.sendMessage(msg.key.remoteJid, {
                text: `❌ Kamu membutuhkan ${formatPrice} balance untuk upgrade ke RAM ${ramSize}GB!\nBalance kamu saat ini: ${formatBalance}`,
                quoted: msg
            });
            return;
        }
        ...
        // Calculate next upgrade details
        const nextRamSize = ramSize + 1;
        const nextRamConfig = RAM_CONFIGS[nextRamSize];
        const nextUpgradePrice = nextRamConfig ? nextRamConfig.price.toLocaleString() : "Max Level";

        const upgradeText = `🎉 *RAM Berhasil Diupgrade!*
        
📦 RAM Sebelumnya: ${ramSize - 1}GB
📦 RAM Baru: ${ramSize}GB

💰 Biaya Upgrade: ${ramConfig.price.toLocaleString()} balance
${nextRamConfig ? `\n📈 Upgrade Selanjutnya (${nextRamSize}GB):
└ Harga: ${nextUpgradePrice} balance` : '\n📈 RAM sudah mencapai level maksimal!'}
