...
for (let i = 1; i <= 30; i++) {
    RAM_CONFIGS[i] = {
        price: Math.floor(1000 * Math.pow(1.5, i-1)), // Exponential price increase
        minReward: 0.0001 * (1 + (i-1) * 0.1),
        maxReward: 0.0005 * (1 + (i-1) * 0.15),
...
Type .buyram <GB> to upgrade your RAM!`;
...
async function buyRamHandler(sock, msg) {
    try {
        ...
        const ramSize = parseInt(args[1]);
        
        if (isNaN(ramSize) || ramSize < 1 || ramSize > 30) {
            await sock.sendMessage(msg.key.remoteJid, {
                text: '❌ Please specify a valid RAM size (1-30GB)\nExample: .buyram 4',
                quoted: msg
            });
            return;
        }
        ...
        if (ramSize <= crypto.ram.size) {
            await sock.sendMessage(msg.key.remoteJid, {
                text: '❌ You already have same or better RAM installed!',
                quoted: msg
            });
            return;
        }
        ...
        if (user.balance < ramConfig.price) {
            await sock.sendMessage(msg.key.remoteJid, {
                text: `❌ You need ${ramConfig.price} balance to buy ${ramSize}GB RAM!\nCurrent balance: ${user.balance}`,
                quoted: msg
            });
            return;
        }
        ...
        const upgradeText = `🎉 *RAM Upgrade Successful!*
        
📦 Previous: ${crypto.ram.size}GB
📦 New: ${ramSize}GB
